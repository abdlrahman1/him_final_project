...
<div class="menu" id = "menu"  onclick="toogle()" > <i class="fa-solid fa-bars" id="ss"></i> القائمة</div>
<ul class="parent-ul">
...
if(isset($_POST["btn"])&&isset($_FILES["img"])&&!empty($_POST["username"])&&!empty($_POST["firstname"])&&!empty($_POST["lastname"])&&!empty($_POST["email"])&&!empty($_POST["password"])) {
    ...
    $_SESSION["name"]=$fname;
    $img2 = pathinfo($img,PATHINFO_EXTENSION);
    $allowd_ex = ["png","jpg","jpeg"];
    ...
    $check_user = $check_query->bindparam(':username',$username);
    $check_query->execute();
    if($check_query->rowCount()>0){
        echo "This email or username address is already registered";
    } else {
        if(in_array($img2,$allowd_ex)){
            if($img_size < 3000000){
